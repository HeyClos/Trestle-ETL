...
- **Promoted_Columns**: The subset of RESO fields materialized as typed MySQL columns on the `property` table in addition to being present in `raw_data`. The initial set is: `ListingKey` (primary key), `ModificationTimestamp`, `StandardStatus`, `MlsStatus`, `PropertyType`, `PropertySubType`, `ListPrice`, `ClosePrice`, `OriginalListPrice`, `ListingContractDate`, `CloseDate`, `StreetNumber`, `StreetName`, `UnitNumber`, `City`, `StateOrProvince`, `PostalCode`, `County`, `Country`, `Latitude`, `Longitude`, `BedroomsTotal`, `BathroomsTotalInteger`, `LivingArea`, `LotSizeSquareFeet`, `YearBuilt`, `DaysOnMarket`, `ListAgentKey`, `ListOfficeKey`, `PhotosCount`, `PublicRemarks`.
...
5. THE `property` table SHALL include secondary indexes on `ModificationTimestamp`, `StandardStatus`, `PropertyType`, `City`, `PostalCode`, `ListPrice`, and `StateOrProvince`.
